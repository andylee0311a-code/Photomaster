...
import { 
  ...
  SlidersHorizontal, Home, Wrench, PlusCircle, Ruler, Lightbulb, Trash2 
} from 'lucide-react';
...
export default function App() {
  // --- 狀態管理 ---
  const [activeTab, setActiveTab] = useState('home'); // home, tools, add
  ...
  const [cropFactor, setCropFactor] = useState(1); // 1: 全片幅, 1.5: APS-C, 2: M43
  ...
  useEffect(() => {
    let interval;
    if (isDetecting) {
      if (luxValue === 0) setLuxValue(1500); // 初始啟動值
      interval = setInterval(() => {
        setLuxValue(prev => {
          const fluctuation = (Math.random() - 0.5) * 800; // 模擬光線波動
  ...
  const getLightSuggestion = (lux) => {
    if (lux === 0) return { desc: '等待測光...', setting: '-' };
    if (lux > 10000) return { desc: '強烈日照', setting: 'ISO 100 / F8 / 1/1000s' };
    if (lux > 2000) return { desc: '明亮陰天或窗邊', setting: 'ISO 400 / F4 / 1/500s' };
    if (lux > 500) return { desc: '一般室內', setting: 'ISO 800 / F2.8 / 1/250s' };
    if (lux > 50) return { desc: '昏暗微光 (廟宇內)', setting: 'ISO 3200 / F1.8 / 1/60s' };
  ...
  const handleAddScenario = (e) => {
    ...
    const newScenario = {
      ...
      icon: <Camera className="w-8 h-8 mb-2" />, // 自訂統一使用相機圖示
      mode: form.mode,
      shutter: form.shutter,
      aperture: form.aperture,
      focusTip: form.focusTip || '無特殊提示',
      bgImage: 'bg-gradient-to-br from-emerald-900 to-teal-950', // 自訂情境專屬綠色系
        ...
        {activeTab === 'tools' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6">
              ...
              <p className="text-xs text-neutral-400 mb-5">避免手震的最低快門速度建議 (安全快門 = 1 / 等效焦距)</p>
              
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-neutral-300 block mb-2">相機片幅</label>
            ...
            <section className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold text-white mb-1 flex items-center gap-2">
                <Lightbulb className={`w-5 h-5 ${isDetecting ? 'text-yellow-400' : 'text-neutral-500'}`} /> 環境測光與曝光建議
              </h2>
              <p className="text-xs text-neutral-400 mb-5">透過鏡頭預測環境亮度，給出基礎起手式參數</p>
